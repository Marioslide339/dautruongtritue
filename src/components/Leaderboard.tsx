...
import { Trophy, Search, CloudLightning, RefreshCw, ArrowLeft, ShieldAlert } from 'lucide-react';
...
export default function Leaderboard({ onBack }: LeaderboardProps) {
  ...
  const fetchCloudData = async () => {
    ...
    try {
      ...
      if (entries && entries.length > 0) {
        ...
        setCloudError('Chưa có dữ liệu trên bảng xếp hạng. Hãy là người đầu tiên tham gia! 🌟');
      }
    } catch (err: any) {
      console.error(err);
      setCloudError(`Không thể lấy dữ liệu: ${err.message || err}.`);
      sound.playWrong();
    } finally {
  ...
  // Sort leaderboard items: Score DESC, TimeSpent ASC
  const getSortedList = () => {
    const sorted = [...cloudEntries].sort((a, b) => {
      if (b.score !== a.score) {
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-xl relative overflow-hidden bubble-shadow-md">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            placeholder="Tìm kiếm theo họ tên, lớp hoặc trường của bé..."
        ...
        <div className="mb-4 flex flex-wrap gap-2 items-center justify-between bg-sky-50/70 border border-sky-100 p-3.5 rounded-2xl text-xs sm:text-sm">
          <span className="text-sky-800 font-bold flex items-center gap-1">
            ...
            <span>Kết nối trực tuyến toàn quốc</span>
          </span>
          <button
          ...
          >
            ...
            <span>{loadingCloud ? 'Đang tải...' : 'Tải lại dữ liệu'}</span>
          </button>
        </div>
        ...
        {loadingCloud && (
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-sm font-bold text-gray-400">Đang đồng bộ dữ liệu từ hệ thống trực tuyến...</p>
          </div>
        )}
        ...
        {cloudError && !loadingCloud && (
          <div className="py-10 text-center max-w-md mx-auto space-y-3 text-amber-800">
            ...
            <p className="text-sm font-extrabold">Chưa có dữ liệu!</p>
